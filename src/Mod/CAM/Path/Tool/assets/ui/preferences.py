...
class AssetPreferencesPage:
    def __init__(self, parent=None):
        ...
        self.import_machine_btn.setToolTip(
            translate(
                "CAM_PreferencesAssets",
                "Copy a .fcm machine file into your CAM/Machines folder (see Machine/MACHINE_FORMAT.md).",
            )
        )
    ...
    def import_machine(self):
        """Import a .fcm file into the user CAM/Machines directory."""
        path, _ = QtGui.QFileDialog.getOpenFileName(
            self.form,
            translate("CAM_PreferencesAssets", "Import machine file"),
            str(MachineFactory.get_config_directory()),
            translate("CAM_PreferencesAssets", "FreeCAD machine (*.fcm);;All files (*.*)"),
        )
        if not path:
            return
        try:
            machine = MachineFactory.load_configuration(path)
            saved_path = MachineFactory.save_configuration(machine)
            display_name = MachineFactory.get_machine_display_name(saved_path.name)
            for row in range(self.machines_list.count()):
                it = self.machines_list.item(row)
                if it and it.data(QtCore.Qt.UserRole) == saved_path.name:
                    it.setText(display_name)
                    return
            item = QtGui.QListWidgetItem(display_name or saved_path.stem)
            item.setData(QtCore.Qt.UserRole, saved_path.name)
            self.machines_list.addItem(item)
        except Exception as e:
            Path.Log.error(f"Import machine failed: {e}")
            QtGui.QMessageBox.critical(
                self.form,
                translate("CAM_PreferencesAssets", "Import failed"),
                translate("CAM_PreferencesAssets", "Could not import machine file: {}").format(str(e)),
            )
