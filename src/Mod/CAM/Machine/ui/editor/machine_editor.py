...
class MachineEditorDialog(QtGui.QDialog):
    ...
    def __init__(self, machine_filename: Optional[str] = None, parent=None):
        ...
        self.import_button = QtGui.QPushButton(translate("CAM_MachineEditor", "Import from file…"))
        self.import_button.setToolTip(
            translate(
                "CAM_MachineEditor",
                "Load a .fcm machine file from disk. Use Save to copy it into your CAM/Machines folder.",
            )
        )
        self.import_button.clicked.connect(self.import_machine_from_file)
    ...
    def import_machine_from_file(self):
        """Load a machine definition from a .fcm file; Save writes it to the user Machines folder."""
        path, _ = QtGui.QFileDialog.getOpenFileName(
            self,
            translate("CAM_MachineEditor", "Import machine file"),
            str(MachineFactory.get_config_directory()),
            translate("CAM_MachineEditor", "FreeCAD machine (*.fcm);;All files (*.*)"),
        )
        if not path:
            return
        try:
            self.machine = MachineFactory.load_configuration(path)
            self.filename = None
            self.is_new_machine = True
            title = translate("CAM_MachineEditor", "Machine Editor")
            if self.machine and self.machine.name:
                title += f" - {self.machine.name}"
            self.setWindowTitle(title)
            ...
            if self.text_mode:
                data = self.machine.to_dict()
                self.text_editor.setPlainText(json.dumps(data, indent=4, sort_keys=True))
            QtGui.QMessageBox.information(
                self,
                translate("CAM_MachineEditor", "Import"),
                translate(
                    "CAM_MachineEditor",
                    "Machine loaded. Click Save to store a copy under your CAM asset path (Machines folder).",
                ),
            )
        except Exception as e:
            QtGui.QMessageBox.critical(
                self,
                translate("CAM_MachineEditor", "Import failed"),
                translate("CAM_MachineEditor", "Could not load machine file: {}").format(str(e)),
            )
